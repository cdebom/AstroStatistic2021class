...
-Num futuro próximo estender a análise para galáxias detectadas por fotometria em banda estreita em z~4.5
...
INTENDED STEPS
1) Get the data from PLAsTiCC Astronomical Classification, available in kaggle.com, inspect it and understand the meaning of each data.
2) The model intended to be used is Random Forest, because of its relative simplicity and comprehensiveness, which will increase the chances of completing the work on time.
3) Try to understand which features could be obtained from the time series data referring to light curves, then organize those features and use them in the model.
